...
public class LoadUtils {
	 ...
	 * @throws IOException
	 */
	public static String upload(String type,Integer pid,MultipartFile fileRecord, HttpServletRequest request) throws IllegalStateException, IOException {
		...
		String realPath = request.getServletContext().getRealPath(INITPATH);
        if(type=="video") {
			realPath+="/channel"+pid+"/video/";
		            }
        if(type=="img") {
        	realPath+="/channel"+pid+"/img/";
        }
        if(type=="file") {
        	realPath+="/channel"+pid+"/file/";
        }
		// ②创建该文件夹，若不存在则创
