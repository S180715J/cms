...
public class ChannelService {
	...
	public int saveChannel(Channel channel, Integer lid) {
		...
		// 查询要添加的频道是否存在
		Channel checkChannel = channelMapper.findChannelByName(channel);
		...
		// 1.将频道添加至表中
		channel.setDatetime(new Date());

		Integer i = channelMapper.svaeChannel(channel);
		...
		// 2.通过pid 查询父频道
		Channel pChannel = channelMapper.findChannelById(channel.getPid(), lid);

		// 3.通过id查询刚刚添加的频道
		Channel dcChannel = channelMapper.findChannelByName(channel);

		// 将父频道的路径与添加的频道名拼接成频道路径
		dcChannel.setCnamepath(pChannel.getCnamepath() + "/" + dcChannel.getCname());
		dcChannel.setCidpath(pChannel.getCidpath() + "/" + dcChannel.getCid());

		// 将添加好的频道 的频道路径字段更新
		Integer j = channelMapper.updateChannel(dcChannel);
